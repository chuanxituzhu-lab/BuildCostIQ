...
- Personnel governance is local and role-driven: `GET /api/personnel` and `POST /api/personnel` are available to project managers and explicitly authorized administrative officers; `POST /api/personnel/authorize` is project-manager-only; `POST /api/personnel/delete` removes an active record while retaining the audit event. When no password is supplied to `POST /api/personnel`, the account starts with `123456`, returned only in that create response and marked for first-login password change. `POST /api/auth/password-change` lets the signed-in person replace their own password; only a hash is stored. `POST /api/personnel/rename` changes only the login/display name while retaining `user_id`, password, role, audit and project records, so the successor can log in with the new name and continue the old account. `POST /api/personnel/roles` supports separate or merged `surveyor` + `site_engineer` assignments. The role and assignment catalogs are returned in the personnel snapshot.
...
- `GET /api/line-contracts` also returns `role_workbench`: the versioned input/output/handoff contract for each role. The contract is UI metadata and does not create a new fact store.
- `GET /api/role-work-products?project_id=...` returns only the current actor's saved role products plus products handed to that actor; project managers can inspect the complete project projection. Each record carries `product_type`, role-specific `fields`, `links.event_id`, `links.evidence_refs`, `links.source_refs`, and `collaboration.handoff_to`.
- `POST /api/role-work-products` saves one role-owned work product after required-field and handoff validation. The actor must hold the role being submitted, and the selected handoff target must be declared by that role's contract. Records are adapter projections linked to Core Event/Evidence/Source/Coordination; `second_amount_ledger` is always `false` and human confirmation remains required where the contract says so.
