...
## v0.8.0-rc9 - 2026-08-29

- 产品收敛为项目经理、造价经理和造价员使用的轻量施工造价闭环 Agent，岗位成果和 CAD 算量保持外部工具边界。
- 新增施工全过程造价成果与审计工作流，以及 G0–G6 合同、技术、实物质量、工程量、价格、授权、结算支付七道闸门。
- 结算就绪度改为从事件事实自动派生，并在事件卡片和经营看板显示缺口、责任人和下一动作。
- 新增开工前清标与成本策划六项检查：招标/中标清标、结构化清单、施工图量依据、0#台账、市场价比较和人材机汇总。
- 新增人工、材料、机械的中标价/市场价独立汇总；市场价不回写合同清单金额。
- 新增材料/实体与检验批、隐蔽验收、检测报告匹配，缺项进入潜在审减队列。

## v0.8.0-rc8 - 2026-08-24

- 新增项目级岗位邀请闭环：项目经理/授权行政人员可生成一次性岗位链接，岗位人员接受后自动加入当前项目名册并进入对应 WebUI；邀请支持有效期、撤销、项目绑定和人员审计。
- 中央 WebUI 支持岗位浏览器入口：岗位电脑无需单独安装 BuildCostIQ，可将项目岗位链接保存为桌面快捷方式；人员交接保留原 `user_id`、密码、成果和审计。

## v0.8.0-rc7 - 2026-08-24

- 仓管员及其他操作岗位的工程事件界面按责任范围只读显示；隐藏蒸馏、创建、Outcome 和无关状态操作。
- P07 证据关联改为底层自动投影：按单据号、材料批次、日志/照片编号、WBS 和位置匹配来源；唯一命中自动显示，多项命中交由造价经理/资料员确认。
- 证据自动投影不复制原件、不写入第二证据库，P07 仍是唯一证据事实源。
- 事件与证据 API 增加岗位边界：仓管等操作岗位不能蒸馏、推进事件、写入证据或登记 Outcome。
- 新增工程事件证据退回闭环：造价经理可从固定待办退回已归类成果，填写退回原因和必补项；岗位工作台显示 `RETURNED` 及补充要求。
- 退回只追加审核状态和反馈，不覆盖原始岗位成果；岗位补交的新成果仍由造价经理重新归类/确认，保留完整审计链。

## v0.8.0-rc6 - 2026-08-24

- 生产/技术/现场岗位成果先保留原始事实；仅造价经理可以标注、组织并发起 Core Event。
- 新增确定性的事件候选接入：按关键词与位置给出可解释候选，不自动创建事件、不自动决策，必须由造价经理确认或退回。
- 为既定工程事件生成按岗位固定的成果要求、交付物和截止时间，并在岗位 WebUI 自动展示，岗位无需手选事件。
- 施工员/测量员新增施工日志日期、日志编号和现场照片编号字段；日志与照片可进入事件候选队列并保留原始来源。
- 新增事件归类权限与审计测试，保持 P01–P08 为事实源，不建立第二金额事实源。
- 仓管员及其他操作岗位的工程事件界面改为按责任范围只读显示；隐藏蒸馏、创建、Outcome 和无关状态操作。
- 新增 P07 证据自动关联投影：按单据号、批次、日志/照片编号、WBS 和位置匹配来源；唯一命中自动投影，多项命中交由造价经理/资料员确认，不复制原件。
