...
POST /api/personnel/password-reset   # 项目经理/授权行政人员生成一次性临时密码
POST /api/auth/password-change       # 已登录人员修改自己的密码
```

原始 token 只保存在邀请链接和服务端哈希中；接受、撤销、过期和项目绑定均写入人员审计。

密码同样采用只写入设计：服务端只保存密码哈希，后台不能查看任何人的原密码。
通过人员管理直接录入的账号，初始密码统一为 `123456`；项目经理或授权行政人员在保存成功时将其安全交给本人，
岗位人员首次登录后点击顶部“修改密码”自行设置新密码。初始密码只在创建响应中显示，不写入人员快照、审计或运行日志。
人员离职、交接或遗失密码时，项目经理/授权行政人员调用 `password-reset`，原密码立即失效，
接口仅在本次响应中返回一次性临时密码；临时密码不会写入审计或运行日志。账号的 `user_id`、
岗位、项目成果和审计历史保持不变，临时密码遗失时再次重置即可。
重置同时撤销该人员已有的浏览器会话，接任人员使用新临时密码重新登录。
