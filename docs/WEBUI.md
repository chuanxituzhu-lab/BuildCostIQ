...
单机模式下，`runtime/` 位于启动 WebUI 的本机。项目部多台电脑使用时只部署一个中央服务节点，终端通过浏览器访问该节点；不要让多个 WebUI 进程直接写同一个网络共享目录。使用环境变量或启动参数配置中央数据根目录：
...
部署适配层把项目状态、不可变原件、分类归档、外部依据、用户权限、备份和锁分别放入中央根目录的 `projects/`、`sources/`、`archive/`、`basis/`、`auth/`、`backups/`、`locks/`。同一项目的读改写由项目锁和 revision 保护；终端不直接改底座文件。`GET /api/deployment` 只返回节点模式、存储逻辑位置和一致性策略，不返回绝对物理路径。断网终端不应被视为权威数据源，后续离线能力必须通过带项目 ID、事件 ID、版本和人工冲突确认的同步队列接入。
...
| 技术/生产 | 图纸、变更、事件、证据、协同；技术负责人负责分包技术边界/方案/交底/放行，生产经理负责分包计划/资源/实物量/纠偏/移交；生产经理另有进度看板，不直接录入 P04 |
| 施工/测量/质量/试验/安全 | 各自现场成果、图纸/事件/证据和责任链；不开放跨岗位检索 |
| 资料员 | 合同/图纸归档、证据、协同，并保留归档检索；不进入成本或零号台账 |
| 采购员 | 合同/清单物资依据、事件、证据、协同；不进入零号台账 |
| 仓管员 | P02 物资清单、收发料事件、物资证据、协同；不显示 P04 零号台账和跨岗位检索 |
...
- `GET /api/deployment` — deployment mode, central-node authority, logical storage roots, and write-consistency policy; no absolute storage path is exposed.
...
- `GET /api/line-contracts` — responsibility-line ownership plus role-level workflow handoff contracts; project data and write operations remain authenticated and project-scoped, while the UI projects only the current role's cards.
