...
### Plugin marketplace and connector workspace

The overview’s TOOL ENTRY section and the top navigation’s “三方工具接入” tab are clickable connector catalogs. Selecting a connector opens a second-level workspace with supported formats, import/export direction, data boundary, and the next action: Excel/CSV opens P02, Word opens local material intake or Word-compatible export, CAD/quantity opens P03 or a project exchange package, budget software uses the exchange package, and government/quota/price-information connectors open the external-basis snapshot intake with the correct category preselected. Local MarkItDown and consent-gated Baidu OCR use the same connector-style detail screen; no external transfer is initiated by opening the screen.
