...
The left work-assist rail places a dedicated “插件中心” button directly below the P08 settlement-review result. It opens a Codex-style local plugin marketplace, where users first choose Excel/CSV, Word, CAD/quantity, budget software, project exchange packages, recognition tools, or external-basis connectors; selecting a card opens the second-level connector workspace. The overview’s TOOL ENTRY section and the top navigation’s “三方工具接入” tab remain clickable connector catalogs. The overview and top navigation open the marketplace first, while a selected card opens the corresponding import, export, local snapshot, or explicit-consent action: Excel/CSV opens P02, Word opens local material intake or Word-compatible export, CAD/quantity opens P03 or a project exchange package, budget software uses the exchange package, and government/quota/price-information connectors open the external-basis snapshot intake with the correct category preselected. Recognition tools, including local MarkItDown and consent-gated Baidu OCR, use the same connector-style detail screen; no external transfer is initiated by opening the screen.
