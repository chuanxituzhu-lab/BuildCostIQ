...
技术负责人（技术边界/方案/交底/放行）
        ↓
生产经理（计划/资源/实物量/纠偏/移交）
        ↓
现场施工·测量·质量·试验（事实与实体验收）
        ↓
造价员（量价基础与证据关联）
        ↓
造价经理（商业总审、计量、结算与 Outcome）
        ↓
项目经理（决策、资源、升级与经营闭环）
```

技术负责人和生产经理是并行责任线，不互相代改事实；造价经理接收两条线的可核验成果并负责商业闸门；项目经理负责跨线决策和升级，不直接修改专业事实。每次交接必须落到现有 `Event → Evidence → Verification → Outcome`，由对应责任线负责人人工确认。
