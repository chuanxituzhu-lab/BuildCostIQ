...
def _personnel_authorize(payload: object, actor: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return AUTH_STORE.authorize_personnel_admin(dict(actor), user_id, authorized)
...
def _personnel_delete(payload: object, actor: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not user_id:
        raise ValueError("缺少目标人员")
    return AUTH_STORE.delete_user(dict(actor), user_id)
...
def _personnel_rename(payload: object, actor: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return AUTH_STORE.rename_user(dict(actor), user_id, new_username)
...
def _personnel_roles(payload: object, actor: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return AUTH_STORE.update_roles(dict(actor), user_id, roles)
...
def _line_contracts() -> dict[str, Any]:
    """Publish the three line contracts without exposing runtime paths."""
    return {
...
def _health() -> dict[str, Any]:
    return {
        ...
        "release_highlights": "P09 全过程成果经营已接入：Event → Evidence → Outcome 双状态机、六段价值泄漏和异常经营队列由现有 P01–P08 事实派生；不建立第二金额事实源",
    }
