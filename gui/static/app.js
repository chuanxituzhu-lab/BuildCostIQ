...
const UI_TRANSLATIONS = {
  ...
  "当前版本": "Current version", "修改密码": "Change password", "退出登录": "Sign out", "登录本地工作台": "Sign in to local workspace",
  "初始登录密码固定为 123456，由项目经理或已授权行政人员交给本人；人员首次登录后请点击顶部“修改密码”自行设置新密码。": "The initial password is fixed at 123456. The project manager or authorized administrative officer hands it to the person, who changes it from the header after first sign-in.",
  "系统只保存密码哈希，后台不能查看原密码。新录入人员初始密码固定为 123456，首次登录后请在顶部“修改密码”自行设置；人员离职或遗失密码时，点击人员行“重置密码”，生成一次性临时密码并立即使原密码失效。": "Only password hashes are stored; the original password cannot be viewed. New personnel start with 123456 and change it from the header after first sign-in. For a departure or lost password, reset the account from the personnel row to issue a one-time temporary password and invalidate the old one.",
  "密码已修改，下次请使用新密码登录": "Password changed. Use the new password next time.",
...
async function changeOwnPassword() {
  const currentPassword = window.prompt(state.language === "en" ? "Enter current password" : "输入当前密码");
  if (currentPassword === null) return;
  const newPassword = window.prompt(state.language === "en" ? "Enter new password (at least 6 characters)" : "输入新密码（至少 6 位）");
  if (newPassword === null) return;
  const confirmPassword = window.prompt(state.language === "en" ? "Re-enter new password" : "再次输入新密码");
  if (confirmPassword === null) return;
  setError("");
  try {
    const response = await apiJson("/api/auth/password-change", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
...
function renderRoleOverview() {
  ...
  $("workspaceContent").innerHTML = `
    ...
    <section class="overview-panel role-manual-panel">
      <div class="surface-title"><div><span class="panel-label">ROLE OPERATION MANUAL · v0.8.0-rc9</span><h3>岗位操作手册</h3></div><span class="surface-caption">费曼四步：说清楚 → 动手做 → 交成果 → 找漏洞</span></div>
...
function renderOverview() {
  ...
  $("workspaceContent").innerHTML = `
    ...
    <section class="overview-panel role-manual-panel">
      <div class="surface-title"><div><span class="panel-label">ROLE OPERATION MANUAL · v0.8.0-rc9</span><h3>造价经理操作手册</h3></div><span class="surface-caption">费曼四步：说清楚 → 动手做 → 交成果 → 找漏洞</span></div>
      <div class="overview-cards role-manual-cards">
        <div class="overview-card"><span class="card-label">先说清楚</span><strong>输入</strong><small>${escapeHtml(manual.start)}</small></div>
        <div class="overview-card"><span class="card-label">动手保存</span><strong>操作</strong><small>${escapeHtml(manual.save)}</small></div>
        <div class="overview-card"><span class="card-label">交付成果</span><strong>协同</strong><small>${escapeHtml(manual.handoff)}</small></div>
...
function renderPersonnelTable(container) {
  ...
  users.forEach((user) => {
    ...
    if (user.id !== state.auth.user?.id) {
      ...
      resetPassword.className = "button button-quiet";
      resetPassword.type = "button";
      resetPassword.textContent = "重置密码";
      resetPassword.title = "不能查看原密码，只能生成一次性临时密码";
      resetPassword.addEventListener("click", () => resetPersonnelPassword(user));
...
function revealPersonnelCredentials() {
  const notice = document.querySelector(".credential-notice");
  if (notice) notice.scrollIntoView({ behavior: "auto", block: "center" });
}
...
function renderPersonnel() {
  ...
  $("workspaceContent").innerHTML =
    ...
    '<div class="notice-line"><strong>密码管理</strong><span>系统只保存密码哈希，后台不能查看原密码。新录入人员初始密码固定为 123456，首次登录后请在顶部“修改密码”自行设置；人员离职或遗失密码时，点击人员行“重置密码”，生成一次性临时密码并立即使原密码失效。</span></div>' +
    credentialNotice +
    ...
    '<div class="permission-note">只需录入姓名和岗位。初始登录密码固定为 123456，由项目经理或已授权行政人员交给本人；人员首次登录后请点击顶部“修改密码”自行设置新密码。</div>' +
...
async function savePersonnel(event) {
  ...
  try {
    ...
    renderPersonnel();
    if (response.temporary_credentials) revealPersonnelCredentials();
    setStatus("人员管理已更新");
  } catch (error) {
...
async function resetPersonnelPassword(user) {
  if (!user?.id || user.id === state.auth.user?.id) return;
  if (!window.confirm(`确认重置“${user.username}”的登录密码吗？原密码将立即失效。`)) return;
  setError("");
  try {
    const response = await apiJson("/api/personnel/password-reset", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ project_id: state.projectId, user_id: user.id }),
    });
    ...
    renderPersonnel();
    if (response.temporary_credentials) revealPersonnelCredentials();
    setStatus("密码已重置，请把一次性临时密码安全交给本人");
  } catch (error) {
    setError(error.message);
  }
}
...
$("changePasswordButton").addEventListener("click", changeOwnPassword);
