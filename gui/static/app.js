...
const ROLE_VIEW_ACCESS = {
  project_manager: ["overview", "dashboard", "search", "events", "p09", "coordination", "connectors", "personnel"],
  cost_manager: ["overview", "search", "contract", "boq", "drawings", "baseline", "plan", "changes", "events", "evidence", "review", "p09", "coordination", "basis", "connectors", "dashboard", "control"],
  cost_estimator: ["overview", "search", "contract", "boq", "baseline", "plan", "changes", "events", "evidence", "coordination", "basis", "connectors"],
...
const ROLE_WORKBENCH_SPECS = {
  ...
  technical_lead: { title: "技术负责人工作台", focus: "图纸、方案、交底、技术核定与专业审核形成技术证据。", outputs: "图纸会审、施工方案、技术交底、技术核定", workflow: ["核图纸", "出方案", "做交底", "专业审核"], execution: "方案未通过技术闸门，不进入现场实施。" },
  production_manager: { title: "生产经理驾驶台", focus: "计划、资源、实物量和偏差纠偏推动现场兑现。", outputs: "总/月/周计划、资源组织、进度纠偏", workflow: ["排计划", "配资源", "盯实物量", "做纠偏"], execution: "只推动生产落地，不代填测量、质量和造价成果。" },
  site_engineer: { title: "施工员/测量员现场工作台", focus: "记录现场事实、完成量和施工事件，成果交技术、质量和造价线验证。", outputs: "现场事实、日完成量、施工事件、基础实测", workflow: ["发现现场", "记录位置时间", "提交完成量", "等待复核"], execution: "事实一次录入，绑定 WBS/位置/事件后提交验证。" },
...
function renderPluginMarketplace() {
  ...
  $("workspaceContent").innerHTML = '<div class="surface-title"><div><span class="panel-label">PLUGIN MARKETPLACE</span><h3>三方工具接入中心</h3></div><span class="surface-caption">像 Codex 插件市场一样选择工具，再进入接入向导</span></div>' +
    '<div class="capability-intro"><strong>本地优先、按需接入。</strong><span>打开目录不会发送任何项目资料；文件交换、外部依据和 OCR 均在下一步由用户明确选择。</span></div>' +
    '<div class="plugin-marketplace-meta"><span>可接入工具</span><strong>' + entries.length + '</strong><span>数据边界</span><strong>本地保存 · 明确授权外发</strong></div>' +
    '<section class="plugin-marketplace-grid"></section>';
  const target = $("workspaceContent").querySelector(".plugin-marketplace-grid");
  target.replaceChildren(...entries.map((connector) => {
    ...
    const kind = document.createElement("span"); kind.className = "connector-kind"; kind.textContent = connector.category === "basis" ? "外部依据" : connector.id.includes("ocr") || connector.id.includes("markitdown") ? "识别工具" : "三方工具";
    const note = document.createElement("small"); note.textContent = connector.description;
    const status = document.createElement("span"); status.className = `connector-status connector-${connector.status}`; status.textContent = connectorCatalogStatus(connector);
    const action = document.createElement("span"); action.className = "connector-open"; action.textContent = "点击进入接入 →";
...
function renderConnectorWorkspace() {
  ...
  $("workspaceContent").innerHTML = `
    <div class="surface-title"><div><span class="panel-label">CONNECTOR WORKSPACE</span><h3>${escapeHtml(connector.name)}</h3></div><button id="backFromConnector" class="button button-quiet" type="button">返回插件中心</button></div>
...
function renderPersonnel() {
  ...
  $("workspaceContent").innerHTML =
    '<div class="surface-title"><div><span class="panel-label">PERSONNEL MANAGEMENT</span><h3>人员管理</h3></div><span class="surface-caption">项目经理直管；行政人员授权后协同；密码仅保存在本机</span></div>' +
    `<div class="notice-line"><strong>人员治理规则</strong><span>${escapeHtml(policyText)} ${escapeHtml(handoverText)}</span></div>` +
    '<div class="control-grid personnel-grid">' +
    '<section class="control-panel"><span class="panel-label">NEW PERSONNEL</span><h3>录入人员</h3>' +
    '<form id="personnelForm" class="work-form"><div class="field-grid"><label>姓名/登录名<input id="personnelUsername" autocomplete="off" required minlength="1" maxlength="64" /></label><label>初始密码<input id="personnelPassword" type="password" autocomplete="new-password" required minlength="6" /></label></div>' +
    `<div class="field-grid"><label>岗位角色<select id="personnelRole">${roleOptions}</select></label><div class="permission-note">新增后仅显示角色和权限摘要，不显示密码。施工员与测量员可在列表中按工程情况合并或拆分；人员增减会写入审计留痕。</div></div>` +
...
async function renamePersonnel(user) {
  ...
  try {
    state.personnel = await apiJson("/api/personnel/rename", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ user_id: user.id, new_username: nextName.trim() }),
    });
...
async function toggleFieldRoleAssignment(user, merged) {
  ...
  try {
    state.personnel = await apiJson("/api/personnel/roles", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ user_id: user.id, roles }),
    });
    syncCurrentPersonnelUser();
    renderPersonnel();
...
async function togglePersonnelAdmin(userId, authorized) {
  ...
  try {
    state.personnel = await apiJson("/api/personnel/authorize", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ user_id: userId, authorized }),
    });
    renderPersonnel();
