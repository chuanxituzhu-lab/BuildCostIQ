...
class LocalAuthStore:
    ...
    def authorize_personnel_admin(self, actor: dict[str, Any], user_id: str, authorized: bool = True) -> dict[str, Any]:
        ...
        self.record_personnel_audit(dict(actor), action, str(user_id), {"username": target.get("username", ""), "authorized": authorized})
    ...
    def rename_user(self, actor: dict[str, Any], user_id: str, new_username: str) -> dict[str, Any]:
        if "manage_personnel" not in set(actor.get("permissions", [])):
            raise PersonnelPolicyError("当前角色没有人员管理权限")
    ...
    def update_roles(self, actor: dict[str, Any], user_id: str, roles: list[str] | tuple[str, ...]) -> dict[str, Any]:
        if "manage_personnel" not in set(actor.get("permissions", [])):
            raise PersonnelPolicyError("当前角色没有人员管理权限")
        user_id = str(user_id)
        users = self._load()
