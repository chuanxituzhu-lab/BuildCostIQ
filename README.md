...
BuildCostIQ version `v0.4.0-rc1` is a local-first construction-cost foundation and project workbench. The architecture is frozen around a small Core, independent adapters, and exactly eight plugin capabilities (`P01`–`P08`).
...
The homepage shows the current release version and the current release highlights: multi-file project intake, local PDF/Office/image recognition and archive classification, explicit external OCR consent, and resumable local project storage.
