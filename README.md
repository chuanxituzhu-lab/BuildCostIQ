...
# 或指定版本
python scripts/bump_version.py 0.8.0-rc3
```
...
The overview and navigation include a clickable “三方工具接入” plugin center. Users can select Excel/CSV, Word, CAD/算量, budget software, the local project exchange package, government basis, quota, price-information, local MarkItDown, or consent-gated Baidu OCR, then enter a second-level flow for the next action. Opening the catalog does not send project data; project managers can inspect it but their action buttons remain read-only.
